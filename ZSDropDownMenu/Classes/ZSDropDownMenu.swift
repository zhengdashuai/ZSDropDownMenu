//
//  ZSDropDownMenu.swift
//  mySwift
//
//  Created by 郑大帅 on 2018/7/24.
//  Copyright © 2018年 郑大帅. All rights reserved.
//
...
public protocol ZSDropDownMenuDataSource: class {
    func numberOfRows(in column: NSInteger, for menu: ZSDropDownMenu) -> Int
...
public extension ZSDropDownMenuDataSource {
    func numberOfColumns(in menu: ZSDropDownMenu) -> NSInteger {
        return 1
    }
...
public protocol ZSDropDownMenuDelegate: class {
    func didSelectRow(at indexPath: ZSIndexPath, for menu: ZSDropDownMenu)
}

public class ZSDropDownMenu: UIView {
    ...
    @objc func menuTapped(paramSender: UITapGestureRecognizer) {
        ...
        for i in 0..<self.numOfMenu where i != tapIndex {
            self.animateIndicator(indicator: indicators[i], forward: false, completion: { _ in
                self.animateTitle(isSelect:false,title: titles[i], show: false, completion: {_ in
                })
            })
...
extension ZSDropDownMenu {
    ...
    func animateTitle(isSelect:Bool, title: CATextLayer, show: Bool, completion: ((Bool) -> Swift.Void)) {
    ...
    func animate(indicator: CAShapeLayer, background: UIView, tableView: UITableView, title: CATextLayer, forward: Bool, completion: @escaping ((Bool) -> Swift.Void)) {
        ...
        if title.foregroundColor == TopSelectTitleColor.cgColor {
            title.foregroundColor = textColor.cgColor
        }else{
            title.foregroundColor = TopSelectTitleColor.cgColor
        }
        animateIndicator(indicator: indicator, forward: forward, completion: {_ in
            animateTitle(isSelect:true,title: title, show: forward, completion: {_ in
...
extension ZSDropDownMenu: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = self.datasource?.titleForRow(at: ZSIndexPath(column: self.currentSelectedMenuIndex, row: indexPath.row), for: self)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.delegate?.didSelectRow(at: ZSIndexPath(column: self.currentSelectedMenuIndex, row: indexPath.row), for: self)
    }
